...
export const eventSlots = mysqlTable('event_slots', {
  ...
  period: varchar('period', { length: 20 }), // "morning", "afternoon", "full_day" (optionnel)
  hourSlot: int('hour_slot'), // Numéro d'heure (0-23) pour faciliter les requêtes
  ...
  priority: int('priority').default(0), // Priorité du créneau (plus élevé = prioritaire)
  isRecurring: int('is_recurring', { unsigned: true }).default(0), // 0 ou 1 - créneau récurrent
...
export const eventRegistrations = mysqlTable('event_registrations', {
  ...
  // Gestion des préférences et contraintes
  isPreferred: int('is_preferred', { unsigned: true }).default(0), // 0 ou 1 - créneau préféré
  canSwitch: int('can_switch', { unsigned: true }).default(1), // 0 ou 1 - peut être réassigné
